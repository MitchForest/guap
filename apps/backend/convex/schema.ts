...
const graphSnapshot = v.object({
  nodes: v.array(
    v.object({
      clientId: v.string(),
      type: literalUnion(WorkspaceNodeKindValues),
      label: v.string(),
  ...
  edges: v.array(
    v.object({
      ...
      kind: v.optional(literalUnion(AutomationEdgeKindValues)),
  ...
  rules: v.array(
    v.object({
      clientId: v.string(),
      sourceClientId: v.string(),
      trigger: literalUnion(WorkspaceRuleTriggerValues),
...
const membershipRole = literalUnion(MembershipRoleValues);

const membershipStatus = literalUnion(MembershipStatusValues);
...
const requestKind = literalUnion(RequestKindValues);

const requestState = literalUnion(RequestStateValues);
...
export default defineSchema({
  ...
  rules: defineTable({
    workspaceId: v.id('workspaces'),
    sourceNodeId: v.id('nodes'),
    triggerType: literalUnion(WorkspaceRuleTriggerValues),
